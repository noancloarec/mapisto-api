...
algeria = Territory.from_dict({
    'd_path' : "M 0.1 0 L 10.12 10 L 0 15.7 L -2 7 L -3 6 L -5 0 Z",# bounding box -5 , 0, 16, 16
    'state_id' : 12, 
...
corsica = Territory.from_dict({
    'd_path' : "M 0.1 10 L 10.12 20 L 0 25.7 L -2 17 L -3 16 L -5 10 Z",# bounding box -5 , 10, 16, 16
    'state_id' : 12, 
...
def test_add_and_get():
    with get_db_with_states_cursor(france_1912_2018) as cursor:
        ...
        france_id = StateCRUD.get_by_name(cursor, 'France', france_1912_2018.validity_start, france_1912_2018.validity_end)[0].state_id
...
def test_get_bbox_in_period_nominal():
    with get_db_with_state_and_territories_cursor(france_1912_2018, algeria, corsica) as cursor:
        bbox = BoundingBox(-20, -2, 16, 14)
        res = TerritoryCRUD.get_within_bbox_in_period(cursor, bbox, year_to_date(1900), year_to_date(1920), 0)
        assert isinstance(res, list)
        assert len(res) >=2
        assert any(t.equals(corsica) for t in res)
        assert any(t.equals(algeria) for t in res)
